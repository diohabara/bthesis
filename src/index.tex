...
\TS{Introduction should include a summary of what this thesis achieves.  The details of the existing methods can be discussed in the related work section, which should be  put before the conclusion.} \TK{move "Related Work"(previous "Existing methods") to after this chapter and add section "Achievement" in this chapter}
  ...
  \section{Program Synthesis}
    ...
    \begin{enumerate}
      \item The first step is to receive specification from a user.
      \item The second step is to line up a candidate program.
      \item The third step is to check if the candidate program meets some requirements.
      \item The last step is, depending on whether it meets the requirements or not, to return feedback to the synthesizer or synthesize the code.
    \end{enumerate}
