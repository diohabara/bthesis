...
\chapter{Method}\label{chapter:method}
  ...
  \section{Syntax}
    ...
    \begin{figure}[htbp]
      ...
      \begin{array}{lccl}
        \emph{Values} &
          \mathtt{v} & ::= & \mathtt{unit \mid boolean \mid integer \mid string }\\
        \emph{Expressions} &
  ...
  \section{Semantics}
    ...
    \subsection{Type System}
      % FIXME: type system
      % TODO: add explanation in the natural language
      Figure \ref{fig:typeSystem} shows the syntax of $\mathbf{Titan}$.
      \begin{figure}[htbp]
        \centering
        \begin{flushleft}
          \textbf{Type System} \quad \framebox{$\Gamma \vdash \mathtt{t : \tau}$}
        \end{flushleft}
        \begin{mathpar}
          % var
          \dfrac
            {\mathtt{x : \tau \in \Gamma}}
            {\mathtt{\Gamma \vdash x : \tau}}
          ...
          \dfrac
            {\mathtt{t_1 : bool \quad t_2 : \tau \quad t_3 : \tau}}
            {\mathtt{if\ t_1\ then\ t_2\ else\ t_3 : \tau}}
            \left(\textrm{T-IF}\right) \and
          % fun
            \dfrac
              {\mathtt{\Gamma, x : \tau_1 \vdash t_2 : \tau_2}}
              {\mathtt{\Gamma \vdash fun\ (x : \tau_1 \to t_2) : \tau_1 \to \tau_2}}
              \left(\textrm{T-ABS}\right) \and
            \dfrac
              {\mathtt{\Gamma \vdash t_1 : \tau_{11} \to \tau_{12} \quad \Gamma \vdash t_2 : \tau_{11}}}
              {\mathtt{\Gamma \vdash t_1\ t_2 : \tau_{12}}}
  ...
  \section{Synthetic Process}\label{section:syntheticProcess}
    % TODO: effect system
    % TODO: add explanation in the natural language
    Figure \ref{fig:synthesisRules} shows the syntax of $\mathbf{Titan}$.
    \begin{figure}[htbp]
      \centering
      \begin{flushleft}
